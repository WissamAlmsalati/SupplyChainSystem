...
export default function Customers() {
  ...
  const customerTypeId = userTypes.find((t) => t.name === 'customer')?.id ?? ''
  ...
  const openCreate = () => {
    setForm({ ...initial, user_type_id: customerTypeId })
    setEditing(null)
    setModal(true)
  }
  ...
  const openEdit = (item) => {
    setForm({
      ...
      ...item,
      password: '',
      user_type_id: item.user_type_id ?? customerTypeId,
    })
    setEditing(item)
    setModal(true)
  }
  ...
  const columns = [
    ...
    { key: 'mobile_number', label: 'الجوال' },
    {
      ...
      label: 'العناوين',
      render: (r) => (
        <Button variant="secondary" size="sm" onClick={() => navigate(`/addresses?user_id=${r.id}`)}>
          العناوين
        </Button>
      ),
    },
      ...
      <Modal title={editing ? 'تعديل مقهى' : 'إضافة مقهى'} open={modal} onClose={close}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input
            ...
            required={!editing}
          />
          <input type="hidden" value={form.user_type_id} />
