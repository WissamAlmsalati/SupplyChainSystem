...
export default function Cart() {
  ...
  useEffect(() => {
    client.get('/customer/addresses')
      ...
      .catch(() => setAddresses([]))
    client.get('/customer/wallet').then((res) => setWalletBalance(Number(res.data?.data?.balance ?? 0))).catch(() => setWalletBalance(null))
  }, [])
  ...
  const handleSaveRecurring = async () => {
    ...
    setError('')
    try {
      await client.post('/customer/recurring-carts', {
