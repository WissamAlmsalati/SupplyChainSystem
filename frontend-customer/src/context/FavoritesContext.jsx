...
export function FavoritesProvider({ children }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const res = await client.get('/customer/favorites/ids')
  ...
  const toggle = useCallback(async (productId) => {
    ...
    try {
      if (wasFavorite) await client.delete(`/customer/favorites/${id}`)
      else await client.post('/customer/favorites', { product_id: id })
    } catch (err) {
      setIds((prev) => {
        const next = new Set(prev)
