...
<head>
    ...
    <title>Customer API QA Dashboard</title>
    <style>
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>Customer API QA Dashboard</h1>
                <div class="subtitle">فحص تلقائي لـ Customer Mobile API endpoints — نجاح + فشل متوقع</div>
            </div>
    ...
    <script>
        const specUrl = '/docs/customer/json';
        ...
        function corruptBody(body) {
            ...
            const idFields = ['branch_id', 'product_variant_id', 'order_id', 'delegate_id', 'customer_id', 'user_id', 'warehouse_id'];
        ...
        function extractIds(path, json) {
            const cartItem = path === '/customer/cart/items' && json.data && json.data.data;
            if (cartItem && typeof cartItem.id === 'number') {
                state.cartItemId = cartItem.id;
                return;
            }
            if (path === '/customer/cart' && json.items && json.items[0] && typeof json.items[0].id === 'number') {
        ...
        async function runTests() {
            ...
            for (const test of tests) {
                ...
                if (!test.negative && test.path === '/customer/cart/checkout' && test.result?.status === 'pass') {
