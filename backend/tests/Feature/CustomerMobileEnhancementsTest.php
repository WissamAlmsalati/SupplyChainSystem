...
class CustomerMobileEnhancementsTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $cart = Cart::create(['user_id' => $this->customerUser->id, 'type' => CartType::Shopping]);
        CartItem::create([
            'cart_id' => $cart->id,
            'product_variant_id' => $this->variant->id,
            'quantity' => $quantity,
    ...
    public function test_customer_can_get_empty_cart(): void
    {
        $token = $this->token();
        $res = $this->getJson('/api/v1/customer/cart', ['Authorization' => "Bearer $token"]);
    ...
    {
        $token = $this->token();
        $res = $this->postJson('/api/v1/customer/cart/items', [
            'product_variant_id' => $this->variant->id,
            'quantity' => 3,
    ...
    public function test_customer_can_update_cart_item_quantity(): void
    {
        ...
        $token = $this->token();
        $res = $this->putJson('/api/v1/customer/cart/items/' . $item->id, [
            'quantity' => 5,
        ], ['Authorization' => "Bearer $token"]);
    ...
    public function test_customer_can_remove_cart_item(): void
    {
        ...
        $token = $this->token();
        $res = $this->deleteJson('/api/v1/customer/cart/items/' . $item->id, [], [
            'Authorization' => "Bearer $token",
        ]);
    ...
    public function test_customer_can_clear_cart(): void
    {
        ...
        $token = $this->token();
        $res = $this->deleteJson('/api/v1/customer/cart', [], ['Authorization' => "Bearer $token"]);
    ...
    public function test_customer_can_checkout_cart(): void
    {
        ...
        $token = $this->token();
        $res = $this->postJson('/api/v1/customer/cart/checkout', [
    ...
    {
        Cart::create(['user_id' => $this->customerUser->id, 'type' => CartType::Shopping]);
    ...
    public function test_customer_cannot_checkout_to_foreign_address(): void
    {
    ...
    public function test_customer_can_see_delegate_location_for_own_order(): void
    {
        ...
        $token = $this->token();
        $res = $this->getJson('/api/v1/customer/orders/' . $order->id . '/delegate', [
            'Authorization' => "Bearer $token",
        ]);
    ...
    public function test_customer_cannot_see_delegate_for_other_customer_order(): void
    {
    ...
    public function test_customer_dashboard_returns_enhanced_stats(): void
    {
        ...
        $token = $this->token();
        $res = $this->getJson('/api/v1/customer/dashboard', ['Authorization' => "Bearer $token"]);
    ...
    public function test_customer_cannot_set_arbitrary_order_status(): void
    {
        $order = $this->makeOrder('pending');

        $token = $this->token();
        $res = $this->putJson('/api/v1/customer/orders/' . $order->id . '/status', [
            'status' => 'cancelled',
        ], ['Authorization' => "Bearer $token"]);
    ...
    public function test_customer_can_confirm_receipt_after_delivery(): void
    {
    ...
    public function test_customer_cannot_confirm_receipt_before_delivery(): void
    {
        $order = $this->makeOrder('pending');
    ...
    public function test_customer_can_request_cancellation_for_pending_order(): void
    {
        $order = $this->makeOrder('pending');

        $token = $this->token();
        $res = $this->postJson('/api/v1/customer/orders/' . $order->id . '/cancel-request', [], [
            'Authorization' => "Bearer $token",
        ]);
    ...
    public function test_customer_cannot_request_cancellation_for_delivered_order(): void
    {
        $order = $this->makeOrder('delivered');
    ...
    public function test_customer_can_delete_address_without_orders(): void
    {
        ...
        $token = $this->token();
        $res = $this->deleteJson('/api/v1/customer/addresses/' . $address->id, [], [
            'Authorization' => "Bearer $token",
        ]);
    ...
    public function test_deleting_address_with_orders_keeps_order_delivery_snapshot(): void
    {
        ...
        $token = $this->token();
        $res = $this->deleteJson('/api/v1/customer/addresses/' . $this->address->id, [], [
            'Authorization' => "Bearer $token",
        ]);
    ...
    public function test_customer_cannot_delete_foreign_address(): void
    {
        $foreignAddress = Address::create([
            'user_id' => AppUser::factory()->customer()->create()->id,
            'name' => 'عنوان آخر',
        ...
        $token = $this->token();
        $res = $this->deleteJson('/api/v1/customer/addresses/' . $foreignAddress->id, [], [
            'Authorization' => "Bearer $token",
        ]);
