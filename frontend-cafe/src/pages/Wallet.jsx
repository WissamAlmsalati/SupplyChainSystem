...
export default function Wallet() {
  ...
  const load = async () => {
    const [w, t, r] = await Promise.all([
      client.get('/cafe/wallet'),
      client.get('/cafe/wallet/transactions', { params: { per_page: 50 } }),
      client.get('/cafe/wallet/topups', { params: { per_page: 50 } }),
    ])
  ...
  const submitRequest = async (e) => {
    ...
    try {
      ...
      const res = await client.postForm('/cafe/wallet/topups', data)
  ...
  const payOnline = async (e) => {
    ...
    setError('')
    try {
      const res = await client.post('/cafe/wallet/topups/gateway', { amount: Number(form.amount) })
  ...
  const cancelTopup = async (topup) => {
    ...
    try {
      await client.post(`/cafe/wallet/topups/${topup.id}/cancel`)
      await load()
    } catch (err) {
